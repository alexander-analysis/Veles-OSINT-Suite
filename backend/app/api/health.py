...
def health(response: Response, db: Session = Depends(get_db)) -> HealthResponse:
    ...
    return HealthResponse(
        ...
        bots={"market": market_bot.status(), "maritime": maritime_bot.status(), "sanctions": {k: sanctions_bot.status()[k] for k in ("active_listings", "index_size", "refreshing")}, "geopolitical": geopolitical_bot.status(), "blockchain": blockchain_bot.status(), "corporate": corporate_bot.status(), "auth": auth_status()} if db_ok else {"auth": auth_status()},
    )
